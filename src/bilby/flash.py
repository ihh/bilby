...
# Removed key_mask since we are not doing causal attention - IH
...
def _query_chunk_flash_attention(chunk_idx, q, k, v):
    q_chunk_len, batch, heads, dim, k_len, v_dim = *q.shape, k.shape[0], v.shape[-1]
...
def _flash_attention(q, k, v):
    ...
    def chunk_scanner(chunk_idx, _):
        ...
        return (chunk_idx + chunk_sizes, _query_chunk_flash_attention(chunk_idx, q_chunk, k, v))
...
def flash_attention(q, k, v):
  ...
  out, _ = _flash_attention(q, k, v)
  return out

@jit
def flash_attention_forward(q, k, v):
    out, lse = _flash_attention(q, k, v)
    return out, (q, k, v, out, lse)

def _query_chunk_flash_attention_backward(q, k, v, o, do, lse):
...
def flash_attention_backward(res, do):
    ...
    def chunk_scanner(carries, _):
        ...
        dq_chunk, dk_chunk, dv_chunk = _query_chunk_flash_attention_backward(q_chunk, k, v, o_chunk, do_chunk, lse_chunk)
